...
namespace SPNATI_Character_Editor
{
	...
	public class Behaviour
	{
		...
		/// <returns></returns>
		public static DialogueLine CreateStageSpecificLine(DialogueLine line, int stage)
		{
			...
			if (!copy.Image.StartsWith(stage + "-") && stage >= 0)
			{
		...
		public void BuildStageTree(Character character)
		{
			...
			foreach (var workingCase in WorkingCases)
			{
				foreach (int s in workingCase.Stages)
				{
					...
					{
						existingCase.Lines.Add(CreateStageSpecificLine(line, s));
					}
				}
			}
		}
