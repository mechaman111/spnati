...
namespace SPNATI_Character_Editor
{
	public partial class CollectibleControl : SubVariableControl
	{
		...
		protected override void OnBoundData()
		{
			...
			{
				_mode = TargetMode.AlsoPlaying;
				SetAlsoPlayingContext();
			}
		...
		protected override void OnBindingUpdated(string property)
		{
			...
			else if (property == "AlsoPlaying" && _mode == TargetMode.AlsoPlaying)
			{
				SetAlsoPlayingContext();
			}
		}
		...
		private void SetAlsoPlayingContext()
		{
			Case context = Data as Case;
			string target = context.AlsoPlaying;
			Character targetChar = CharacterDatabase.Get(target);
			if (targetChar == null)
			{
				_wildcardVariable = $"~_.collectible.*~";
				recField.RecordContext = null;
			}
			else
			{
				_wildcardVariable = $"~{CharacterDatabase.GetId(targetChar)}.collectible.*~";
				recField.RecordContext = targetChar;
			}
		}
