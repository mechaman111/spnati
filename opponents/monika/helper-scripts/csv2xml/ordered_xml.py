...
class OrderedXMLElement(object):
    ...
    def __serialize(self, level=0, escape_text=True):
        ...
        if self.type is None: # Comment
            ...
            if escape_text:
                out_text = XMLUnescape(self.text, TEXT_UNESCAPES, TEXT_UNESCAPE_RE)
                out_text = XMLEscape(out_text, TEXT_ESCAPES, TEXT_ESCAPE_RE)
            else:
                out_text = self.text
                
            return ["<{:s}{:s}>{:s}</{:s}>".format(self.type, attr_string, out_text, self.type)]
