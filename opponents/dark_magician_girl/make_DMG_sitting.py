...
def create_images(image_list, output_directory, kkl_version):
    ...
    for image_filename, image_data in image_list:
        ...
        image_kkl_filename = kkl_basename +(".png" if kkl_version < 54 else ".png")
...
def make_images(image_data_file, output_directory, kkl_version):
    image_data_list = read_image_data(image_data_file)
    create_images(image_data_list, output_directory, kkl_version)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 3:
        try:
            kkl_version = int(sys.argv[3])
        except ValueError:
            print("Invalid Kisekae version supplied (not a number), defaulting to %d" % kkl_version)
    make_images(sys.argv[1], output_directory, kkl_version)
