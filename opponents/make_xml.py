...
def write_xml(data, filename):
	...
	o.insert(0, ET.Comment("This file was machine generated by make_xml.py version 1.47 in " + mydate.strftime("%B") + " " + mydate.strftime("%Y") +". Please do not edit it directly without preserving your improvements elsewhere or your changes may be lost the next time this file is generated."))
	...
	#label
        for prop in ("label", "intelligence"):
	        for stage in data[prop]:
		        if stage == 0:
			        ET.SubElement(o, prop).text = data[prop][stage]
		        else:
			        ET.SubElement(o, prop, stage=stage).text = data[prop][stage]
...
def read_player_file(filename):
	...
	for line_number, line in enumerate(f):
        ...
        #   The label can be changed in the same manner
			...
			(stage, value) = (0 if len(parts) == 1 else parts[1], parts[0])
			if key in d:
				d[key][stage] = value
			else:
				d[key] = {stage: value}
